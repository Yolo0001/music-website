...
const HttpManager = {
  ...
  // 返回标题包含文字的歌单
  getSongListOfLikeTitle: (keywords, userId) => {
    const url = userId ? `songList/likeTitle/detail?title=${keywords}&userId=${userId}` : `songList/likeTitle/detail?title=${keywords}`;
    return get(url);
  },
  // 返回歌单里指定歌单ID的歌曲
  ...
  // 返回指定歌手名的歌曲
  getSongOfSingerName: (keywords, userId) => {
    const url = userId ? `song/singerName/detail?name=${keywords}&userId=${userId}` : `song/singerName/detail?name=${keywords}`;
    return get(url);
  },
  // 下载音乐
  downloadMusic: (url) => get(url, { responseType: "blob" }),
  // 记录用户播放行为
  recordSongPlay: ({songId, userId}) => post(`song/play`, {songId, userId}),

  // =======================> 搜索历史 API
  // 记录搜索历史
  recordSearchHistory: ({userId, keyword, searchType}) => post(`search/history/record`, {userId, keyword, searchType}),
  // 获取用户搜索历史
  getUserSearchHistory: (userId, limit) => get(`search/history/user?userId=${userId}&limit=${limit || 10}`),
